...
# Streamlit App
st.title("Yasmin's money tracker")
...
if budget > 0:
    ...
    if remaining >= 0:
        st.success(f"You're within budget. Remaining: ${remaining}")
    else:    
        st.error(f"You have exceeded your budget by $ {abs(remaining)}")
else:
    st.info("No budget entred yet")
